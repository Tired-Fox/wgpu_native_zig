...
pub fn write24BitBMP(io: std.Io, file_name: []const u8, comptime width: u32, comptime height: u32, bgra_data: *[width * height * 4]u8) !void {
    ...
    // ID
    _ = try interface.write(&[2]u8{'B', 'M'});
    ...
    try interface.writeInt(u32, file_size, .little);

    // reserved
    try interface.writeInt(u32, 0, .little);
    // data offset
    try interface.writeInt(u32, 54, .little);
    // info size
    try interface.writeInt(u32, 40, .little);
    // image width
    try interface.writeInt(u32, width, .little);
    // image height
    try interface.writeInt(u32, height, .little);
    // Planes
    try interface.writeInt(u16, 1, .little);
    // bits per pixel
    try interface.writeInt(u16, 24, .little);
    ...
    try interface.writeAll(&([_]u8{0} ** (4 * 6)));
    ...
    for (0..height) |i_y| {
        ...
        _ = try interface.write(&line_buffer);
    }
    try interface.flush();
}
