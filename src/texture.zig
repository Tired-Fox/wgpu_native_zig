...
pub const TextureBindingViewDimension = extern struct {
    chain: ChainedStruct = .{
        .s_type = SType.texture_binding_view_dimension
    },
...
pub const TextureComponentSwizzleDescriptor = extern struct {
    chain: ChainedStruct = .{
        .s_type = SType.texture_component_swizzle_descriptor
    },
...
pub const ExternalTextureBindingEntry = extern struct {
    chain: ChainedStruct  = .{
        .s_type = SType.external_texture_binding_entry
    },
...
pub const ExternalTextureBindingLayout = extern struct {
    chain: ChainedStruct  = .{
        .s_type = SType.external_texture_binding_layout
    },
};
...
pub const ExternalTexture = extern struct {
    pub inline fn setLabel(self: *TextureView, label: []const u8) void {
        wgpuExternalTextureSetLabel(self, StringView.fromSlice(label));
    }
...
extern fn wgpuTextureGetNativeMetalTexture(texture: *Texture) *anyopaque; 
...
pub const Texture = opaque {
    ...
    pub inline fn getTextureBindingViewDimension(self: *Texture) TextureViewDimension {
    ...
    /// Extracts low-level Apple Metal texture (MTLTexture)
    pub inline fn getNativeMetalTexture(texture: *Texture) *anyopaque {
        return wgpuTextureGetNativeMetalTexture(texture);
    }
};
