...
pub const ShaderRuntimeChecks = enum(u32) {
    none = 0x0000,
    bounds_checks = 0x0001,
    force_loop_bounding = 0x0002,
    ray_query_initialization_tracking = 0x0004,
    task_shader_dispatch_tracking = 0x0008,
    mesh_shader_primitive_indices_clamp = 0x0010,
};
...
extern fn wgpuDeviceStartGraphicsDebuggerCapture(device: *Device) WGPUBool;
extern fn wgpuDeviceStopGraphicsDebuggerCapture(device: *Device) void;
...
extern fn wgpuDeviceCreateShaderModuleTrusted(device: *Device, descriptor: *const ShaderModuleDescriptor, runtime_checks: ShaderRuntimeChecks) ?*ShaderModule;

extern fn wgpuDeviceGetNativeMetalDevice(device: *Device) *anyopaque;
...
pub const Device = opaque {
    ...
    pub inline fn startGraphicsDebuggerCapture(self: *Device) bool {
        return @bitCast(wgpuDeviceStartGraphicsDebuggerCapture(self));
    }
    ...
    pub inline fn createShaderModuleTrusted(
        ...
        return wgpuDeviceCreateShaderModuleTrusted(self, descriptor, runtime_checks);
    }

    /// Extracts low-level Apple Metal device handle (MTLDevice)
    pub inline fn getNativeMetalDevice(self: *Device) *anyopaque {
