...
pub const Adapter = opaque{
    ...
    pub fn requestDeviceSync(self: *Adapter, instance: *Instance, descriptor: ?*const DeviceDescriptor, io: Io, duration: Io.Duration) RequestDeviceResponse {
...
test "can request device" {
    ...
    const adapter_response = instance.?.requestAdapterSync(null, .fromMilliseconds(200));
    const adapter: ?*Adapter = switch(adapter_response.status) {
        .success => adapter_response.adapter,
        else => null,
    };
    const device_response = adapter.?.requestDeviceSync(instance.?, null, .fromMilliseconds(200));
